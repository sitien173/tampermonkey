...
(function () {
  ...
  function createSettingsPanel() {
    ...
    panel.innerHTML = `
            ...
            <div style="margin-bottom: 15px;">
                ...
                <input type="text" id="worker-url" value="${
            ...
            <div style="margin-bottom: 15px;">
                <label style="display: flex; align-items: center; cursor: pointer;">
                    <input type="checkbox" id="enable-cache-response" ${
                      config.enableCachedResponses ? 'checked' : ''
                    } style="margin-right: 8px;">
                    Cache Responses
                </label>
            </div>
            ...
            <div style="margin-bottom: 15px;">
                <label style="display: flex; align-items: center; cursor: pointer;">
                    <input type="checkbox" id="show-ui-buttons" ${
                      config.showUiButtons ? 'checked' : ''
                    } style="margin-right: 8px;">
                    Show UI Button
                </label>
            </div>
  ...
  function normalizeMonthOfBirth(input) {
    ...
    if (complexMatch) {
      ...
      const month = dobMonthOpts.find(
        (m) => normalizeInput(m.text) === monthText.trim() || m.value === parseInt(monthValue)
      );
  ...
  function detectContext() {
    ...
    if (isVerification) {
      ...
      if (el) {
        const text = isKyb
          ? document.querySelector('[data-testid=country-selection-inputbox] #search')?.value
          : el.textContent;
  ...
  function handleSearchField(responseMap) {
    ...
    document.querySelectorAll('input[name=search]').forEach((searchInput) => {
      ...
      document.querySelectorAll('[id$=-dropdown-menu]').forEach((dropDown) => {
        ...
        if (option) {
          option.dispatchEvent(new MouseEvent('click', { bubbles: true, cancellable: true }));
        }
      });
    });
  }
  ...
  async function fetchExtractedDataFromClipboard(fields) {
    ...
    const res = await postJson(`${config.BACKEND_ENDPOINT}/api/extracting-data`, payload);
