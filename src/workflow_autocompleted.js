...
(function () {
  ...
  function extractReactSelectIndex(element) {
    ...
    const ariaDescribedByIndex = extractIndexFromString(element.getAttribute?.('aria-describedby'));
  ...
  function buildSelectorsForElement(elementDefinition) {
    ...
    if (role) {
      selectors.push(`[name="${role}"]`, `[id$="${role}"]`, `[data-role="${role}"]`);
    }
    if (name) {
      selectors.push(
  ...
  function addTypeSpecificControls(controls, elementDefinition) {
    ...
    if (type === 'phone_number') {
      const numberInput = document.querySelector(`[data-testid="text-field-input-${id}-number"]`);
    ...
    if (type === 'email') {
      const emailInput = document.querySelector(`[data-testid="text-field-input-${id}"]`);
  ...
  async function collectCountryOptions() {
    ...
    if (isReactSelectControl(primaryControl)) {
      ...
      const { index, listbox } = context;
      const options = getReactSelectOptions(listbox, index);
      values = options.map((option) => option.textContent?.trim()).filter(Boolean);
